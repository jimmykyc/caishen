...
setInterval(function () {
    ;(async () => {
        ...
        accountBalance = await balanceHandler();
       	tokensReserved = (accountBalance * 7500000).toFixed(2);
    })()
    ...
    $("#totalRaised").text(total.toFixed(2) + " / " + cap + " ETH (" + progress + "%)");

    $("#userRaised").text(accountBalance+' ETH');
			...
			await c.methods.depositBNB().send({
				...
				if(callback.hasOwnProperty("blockHash")) {
					...
					setTimeout(function() {
						...
						$('#resultMsg').load('https://usagi-protocol.com/privatesale/send_presale_buy_message_eth.php?amount='+$('#bnbAmount').val()+'&total_raised='+total);
						$('#refMsg').load('https://usagi-protocol.com/privatesale/referral.php?chaintype=eth&amount='+$('#bnbAmount').val()+'&walletadress='+address+'&ref='+$('#refwalletaddress').val());
					}, 5000);
